...
from base import ForecastResult, DeterministicForecastResult, Signal
...
#
#   where g(t) is a deterministic process
#
...
#
# class DeterministicModel(CompositeSignal):
#
#     #   y_t - B u_t = A g(t) + L e_t,    e_t ~ N(0, I)
#
#     def __init__(self,
#                  deterministic_process: "DeterministicProcess",
#                  deterministic_loadings: ConstrainedMatrix,
#                  exog: Optional[np.ndarray] = None,
#                  exog_loadings: Optional[ConstrainedMatrix] = None):
#
#         self._deterministic_process = deterministic_process
...
#             exog_shape = (
#                 BaseUtils.require_equal_or_none([self.shape[1], self.exog_loadings.shape[1]], True))
#
#             self.exog_loadings.shape = (None, exog_shape)
#             obs_shapes.append(self.exog_loadings.shape[0])
#
#
#         deterministic_process_shape =\
#             BaseUtils.require_equal_or_none([self.deterministic_process.shape[0], self.deterministic_loadings.shape[1]], True)
#         self.deterministic_loadings.shape = (None, deterministic_process_shape)
#
#         obs_shapes.append(self.deterministic_loadings.shape[0])
#
#         obs_shape = BaseUtils.require_equal_or_none(obs_shapes, False)
#
#         if len(obs_shape):
#             if self.exog is not None:
#                 self.exog_loadings.shape = (obs_shape, exog_shape)
#
#             self.deterministic_loadings.shape = (obs_shape, exog_shape)
#
#         return obs_shape
#
#     @property
#     def _underlying_signals(self) -> list[Signal]:
#         signals = [self.deterministic_process]
#         if self.exog is not None:
...
#
#     @property
#     def deterministic_process(self) -> "DeterministicProcess":
...
#
#
# class DeterministicProcess(Model):
#
#     def __init__(self, shape: tuple, **signal_init):
#         super().__init__(shape, **signal_init)
#
#     @abstractmethod
#     def __call__(self, time_domain: Union[tuple[int, int], int]):
...
#
#     @abstractmethod
#     def forecast(self, time_domain: Union[tuple[int, int], int]):
#         raise NotImplementedError
#
...
# class ConstantProcess(DeterministicProcess):
#
#     def __int__(self, shape: tuple[int, int]):
#         super().__init__(shape)
#         self._constant = UnConstrained(shape)
#
#     @property
#     def value(self):
#         return self._constant.matrix
#
#     @value.setter
...
#
#     def __call__(self, time_domain: Union[tuple[int, int], int]):
#         return self.forecast(time_domain)
#
#     def _update_params(self, params: np.ndarray) -> None:
#         self._constant.update_params(params)
#
#     def _get_params(self) -> np.ndarray:
#         return  self._constant.get_params()
#
#     @property
#     def is_defined(self):
#         return self._constant.is_defined
#
#     def forecast(self, time_domain: Union[tuple[int, int], int]):
...
#             T = time_domain
#
#         return np.broadcast(self._constant, np.zeros((T, *self._constant.shape)))
#
#     def fit(self, series: np.ndarray) -> DeterministicModelFit:
#         pass
#
#     @property
#     def n_params(self) -> int:
...
# class LinearTimeTrend(DeterministicProcess):
#
#
# # ------------------------------
# # Polynomial
